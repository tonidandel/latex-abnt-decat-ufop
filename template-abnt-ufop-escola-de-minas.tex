...
%% 28.09.2026 19h56 Danny Tonidandel
...
% ---
\titulo{Um Estudo sobre a Dinâmica de Sistemas Complexos \\ {\normalsize \textcolor{red}{Modelo ABNT e Tutorial para TCCs - DECAT-UFOP}} \\ {\small \textcolor{blue}{v26.08}}}
\autor{Maria da Cunha}
\local{Ouro Preto}

% imprime o ano corrente no campo data
\data{\the\year} 

% Altera o rótulo do orientador
\orientador[Orientadora:]{Profa. Dra. Chiquinha Gonzaga}

% Define rótulo para co-orientador
...
\hypersetup{
    	...
    	linkcolor=darkgray,    % cor de links internos
    	citecolor=darkgray,    % cor de links na bibliografia
    	filecolor=magenta, % color de links para arquivos
		urlcolor=darkgray,     % cor de hiperlinks (urls)
}
\makeatother
% ---
...
\newtheorem{especificacao}{Especificação}
	...
	\centering
		\begin{minipage}{\textwidth}
				\begin{tabular}{ccc}
					\begin{tabular}{c}
						\\
						{\includegraphics[width=1.2cm]{logo-universidade.jpg} }
					\end{tabular}
					&
					\begin{tabular}{c}
						\\
					...
					\begin{tabular}{r}
						\\
						{\raggedright \includegraphics[width=2.8cm]{logo-unidade-2.jpg}}
					\end{tabular}
				\end{tabular}
		\end{minipage}
...
% Folha de aprovação - [OBRIGATÓRIO pela NBR 14724/2011]
...
\section{Contextualização}
	O trabalho final de curso ou trabalho de conclusão de curso, popularmente chamado de TFC ou TCC, é um tipo de trabalho acadêmico, assim como monografia de especialização, dissertação de mestrado ou tese de doutorado. Embora no caso dos trabalhos de graduação não seja exigida originalidade do trabalho, ele deve ser realizado em profundidade, sempre com a orientação de um(a) docente orientador(a), buscando uma contribuição válida para o conhecimento humano, na área em que se especializa. 
...
\section{Equações}
	...
	$\Rightarrow$ \textbf{Lembre-se:} \textbf{\textcolor{red}{equações fazem parte do texto e, por isso, devem ser pontuadas!}}
...
\section{Figuras} % exemplo de ambiente sem numeração
	...
	%
	\begin{lstlisting}[caption={Exemplo de código para inserção de figuras.}, label={cod:fig308}]
	\begin{figure}[ht] % Outras opções: b (bottom),t (top),p (page) ou alguma combinação entre elas
	...
	$\Rightarrow$ \textbf{Lembre-se:} \textbf{\textcolor{red}{ Quando o autor do trabalho é o autor da ilustração, a fonte deve ser igualmente citada!}}.

\section{Tabelas}

\subsection{Alternativa 1: tabela elaborada em planilha eletrônica}
...
\subsection{Alternativa 2: Tabela elaborada no ambiente table}
